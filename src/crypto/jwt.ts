...
class Jwt {
   ...
   *
   * @example
   * const jwtToken = JWT.create({ sub: "123" }, "secret", { algorithm: "HS512", expiresIn: "1h" });
   */
  public create(
    payload: JWTPayload,
    ...
    if (options.expiresIn !== undefined) {
      const secs = this.parseExpiresIn(options.expiresIn);
    ...
    const signature = this.signData(algorithm, data, secret);
   ...
   */
  public verify(token: string, secret: string | Buffer): JWTPayload | null {
    try {
      ...
      const data = `${encodedHeader}.${encodedPayload}`;
      const validJWT = this.verifyData(alg, data, secret, signature);
   ...
   *
   * @example
   * conste decodeToken = JWT.decode("token");
   */
  public decode(
    token: string,
   ...
   */
  private base64ToBase64Url(b64: string): string {
  ...
  private verifyData(
    ...
    if (algorithm.startsWith("HS")) {
      const expectedB64 = createHmac(hmacAlgorithm(algorithm), key)
        .update(data)
        .digest("base64");
      const expectedBuf = this.base64UrlToBuffer(
        this.base64ToBase64Url(expectedB64),
      );
      const sigBuf = this.base64UrlToBuffer(signature);
...
/**
 * JWT utility for creating, verifying, and decoding JSON Web Tokens using Node.js crypto.
 *
 * This class implements a compact JWT workflow:
 * - **create()** builds a signed token using a supported `alg` (HS* / RS*),
 *   automatically injecting `iat` and optionally `exp`.
 * - **verify()** validates structure, verifies the cryptographic signature, and enforces expiration.
 * - **decode()** performs a *non-validating* decode for inspection/debugging only.
 *
 * Supported algorithms are defined by the `Algorithm` union and resolved to Node.js crypto
 * primitives (HMAC for HS*, RSA sign/verify for RS*). Base64URL transformations are handled
 * internally to match JWT encoding requirements (RFC 7515 / JWS).
 *
 * Security notes:
 * - JWTs produced here are **signed, not encrypted**. Payloads are readable by anyone who has the token.
 * - For HS* algorithms, verification uses **constant-time comparison** to reduce timing attack risk.
 * - For RS* algorithms, verification relies on RSA public/private key pairs through Node.js crypto.
 *
 * Design notes:
 * - The class is **stateless**: it does not persist sessions or token revocation state.
